...
\qa{Cartesian Representation ($y = ax + b$) vs. Normal Representation ($x \cos\theta + y \sin\theta = \rho$)}
{When building the parameter space for the Hough Transform, why is the normal representation preferred over the standard Cartesian equation?
\begin{itemize}
...
\qa{Coarse Quantization (Few Cells) vs. Fine Quantization (Many Cells)}
{In the Hough Transform algorithm, the parameter space ($\rho$, $\theta$) must be quantized into accumulation cells. What is the engineering trade-off between using few cells versus many cells?
\begin{itemize}
...
\qa{Standard Hough Transform vs. Generalized Hough Transform}
...
\qa{Naive Edge Coupling vs. Hough Transform (Computational Complexity)}
{Why do we use the Hough Transform instead of a naive approach to find lines?
\begin{itemize}
