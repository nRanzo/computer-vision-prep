...
When an image is transformed into the frequency domain, its spatial intensity variations are represented as sinusoidal waves of varying frequencies. Low frequencies correspond to areas of the image where pixel intensities change slowly / are constant, i.e. smooth gradients, uniform background / illumination. They contain the vast majority of the image's energy and appearance. High frequencies correspond to areas of rapid intensity transitions i.e. edges, fine details, random noise. Removing high frequencies results in blurring, while emphasizing them sharpens the image.
}
...
The BLPF introduces a smooth transition between the passed and attenuated frequencies, controlled by the filter's order. Because it avoids a sudden cutoff, it significantly minimizes / completely eliminates the ringing effect -> handy for image restoration.
}
