\lecture{Lecture 9: Morphological ops and segmentation by thresholding}
...
\begin{itemize}
    \item \textbf{Opening} is defined as an Erosion followed by a Dilation: $A \circ B = (A \ominus B) \oplus B$. 
    \item \textbf{Closing} is defined as a Dilation followed by an Erosion: $A \bullet B = (A \oplus B) \ominus B$.
\end{itemize}
}
...
\qa{When segmenting an image, why might variable thresholding be preferred over global thresholding?}{
\begin{itemize}
    \item \textbf{Global thresholding} uses a single threshold value for the entire image, which relies on a clear overall histogram. It often fails when the image suffers from noise or uneven illumination.
    \item \textbf{Variable (or non-global) thresholding} processes different regions of the image independently, calculating different thresholds for each sub-image. This effectively handles local variations like illumination changes where a single global threshold would fail.
\end{itemize}
}

\qa{How does Otsu's method use inter-class variance versus intra-class variance to find the optimal threshold?}{
Otsu's method automatically evaluates all possible thresholds $k$ to divide the image into two classes. 
\begin{itemize}
    \item It aims to \textbf{maximize the inter-class variance} ($\sigma_B^2$), which measures the difference or distance between the two classes. 
    \item By maximizing the inter-class variance, it simultaneously \textbf{minimizes the intra-class variance} ($\sigma_{in}^2$), because the global variance of the image is fixed ($\sigma_G^2 = \sigma_{in}^2 + \sigma_B^2$). 
\end{itemize}
}
